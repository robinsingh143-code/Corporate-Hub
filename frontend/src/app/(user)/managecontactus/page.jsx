...
const Managecontactus = () => {
  ...
  const featchUserList = () => {
    axios.get('http://localhost:5000/contactus/getall')
    .then((res) => {
  ...
  const deleteUser = (id) => {
    axios.delete('http://localhost:5000/contactus/delete/' + id)
    .then((result) => {
  ...
  return (
    <div className='lg:max-w-[80%] mx-auto py-10'>
      <div className='border rounded-xl shadow-lg p-8'>
        ...
        <h1 className='text-center font-bold text-3xl'>manage contact </h1>
        <hr />
              ...
              userList.map((user) => {
                ...
                return <tr key={user._id}>
                  ...
                  <td className='p-2 border border-violet-700'>{user.name}</td>
                    ...
                    <td className='p-2 border border-violet-700'> {user.subject} </td>
                    <td className='p-2 border border-violet-700'>{user.message} </td>
                    ...
                    {/* <td className='p-2 border border-violet-700'>
                      <Link href={'/Updatecontactus/' + user._id} className='bg-blue-500 text-white px-4 py-2 rounded-lg'>Update</Link>
                    </td> */}
                </tr>
              })
            }
